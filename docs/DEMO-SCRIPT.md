...
**Total runtime:** 3 minutes (strict)

## Sponsors Used (6 total)
- **AWS Bedrock** - Agent LLM backbone
- **AWS S3 Vectors** - Attacker fingerprint storage
- **Auth0** - M2M tokens + FGA permissions
- **Tonic Fabricate** - Synthetic honeypot credentials
- **Yutori** - Autonomous threat simulation
- **TinyFish AgentQL** - Attack pattern extraction
...
> **[SPONSOR: Yutori]** "We're using **Yutori's autonomous browsing API** to simulate an attacker agent - it doesn't know which agents are real, it's probing autonomously."
...
> **[SPONSOR: TinyFish]** "Behind the scenes, **TinyFish AgentQL** is extracting structured patterns from this conversation - intent, targets, techniques - all parsed automatically."
...
> **[SPONSOR: Tonic Fabricate]** "These credentials are generated by **Tonic Fabricate** - AI-powered synthetic data that looks production-real but leads to monitored traps. Every credential is unique, follows real naming patterns, but triggers alerts when used."
